...
{
    //
    public function getInfoEvent($indice,$lang)
    {

// 
 $galeria=GalleryModels::where('id',$indice)->where('lang',$lang)->first();
        $slider=GalleryImageModels::where('id_galeria',$galeria->idbase)->get();
//$galeria=$galeria[0];
                ...
                <div class="row no-gutters">
                    <div class="col-12 col-sm-12 col-md-6 col-lg-6 col-xl-6 clsBackColorNuEv clsFLX">
                        <div class="clsContTextoNuestEvent">
                            <div class="clsTextoInfo clsTextoInfo-' . $galeria->idbase . '">
                                '.$galeria->titulo.'
                            </div>
                            <div class="clsPieTexto clsPieTexto-' . $galeria->idbase . '">
                                '.$galeria->contexto.'
                            </div>
                        </div>
                    </div>
...
foreach ($slider as $item) {
    ...
    $code.='<div class="sp-slide">
        <img class="sp-image" src="plugins/slider-pro/css/images/blank.gif"
             data-src="'.$item->img.'"
             data-small="'.$item->img.'"
             data-medium="'.$item->img.'"
             data-large="'.$item->img.'" alt="evento"/>
    </div>';
}
foreach ($slider as $item) {
