...
<div class="clsContFechasBlog container">
    <div class="titulo-entradas-blog">{{$item[3]}}</div>
    <a id="tentradas" href="/blog" align="right">{{$item[4]}}</a>
    <hr>
                    ...
                    <div class="card-body">
                        @foreach (App\Http\Controllers\FuncionesController::getCountMonth($anio->anio) as $mes)
                            ...
                            <div class="cont-mes-publicacion"><a
                                    href="/blog/{{ $anio->anio }}/{{ App\Http\Controllers\FuncionesController::getDateFormat($anio->anio . '-' . $mes->mes . '-1', 'MFl',Session::get('lang')) }}">
                                    {{ App\Http\Controllers\FuncionesController::getDateFormat($anio->anio . '-' . $mes->mes . '-1', 'MF',Session::get('lang')) . '(' . $mes->total . ')' }}
                                </a></div>
                        @endforeach
                    </div>
                </div>
